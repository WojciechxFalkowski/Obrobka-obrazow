<template>
  <div class="home d-flex flex-column">
    <div class="mb-4">
      <button class="btn btn-primary" @click="boxFiltering">Box filtering</button>
      <button class="btn btn-primary" @click="unsharpMasking">Unsharp masking</button>
      <button class="btn btn-primary" @click="bilateralFiltering">Bilateral filtering</button>
      <button class="btn btn-primary" @click="saltAndPepper">SaltAndPepper</button>
    </div>
    ...
    <div class="container-fluid">
      <div class="row pb-5" v-for="imageModel of getImagesCollection" :key="imageModel.id">
        <div class="col-8 d-flex home__image-wrapper pb-2" ref="imageWrapperRef">
          ...
          <!--      <canvas class="border" id="outputCanvas1" width="512" height="512"></canvas>-->
          <!--      <canvas :key="img.id" :id="`abc${img.id}`" v-for="(img) of images" class="border" width="512" height="512">-->
...
export default {
  ...
  mounted() {
    ...
    // const canvasOutputId = "canvasOutput";

    // let src = window.cv.imread('canvasInput');
...
//     console.log(src)
//     console.log(dst)
//     console.log(window.cv.COLOR_RGBA2GRAY)
//     src.delete();
//     dst.delete();
  ...
  methods: {
    ...
    boxFiltering() {
      ...
      this.getImagesCollection.forEach(imageModel => {
        ...
        // let src = window.cv.imread(this.$refs.defaultImageRef);
        activeImages.forEach(image => {
          const imgEl = new Image();
          imgEl.src = image.path;
          // let src = image.path;
          let src = window.cv.imread(imgEl);
          ...
          // window.cv.imshow("outputCanvas1", dst);
          // dst.delete() ;
          // ksize.delete() ;
          // anchor.delete() ;
          // const img = {
          ...
          // console.log(dst.data)
          this.createCanvas(dst, imageModel.id, imageModel.images.length)
    ...
    createCanvas(dst, imageModelId, imageId) {
      console.log('dst')
      console.log(dst)
      const canvas = document.createElement('canvas');
    ...
    unsharpMasking() {
      ...
      let kernelSize = 3;
      let kernel = window.cv.matFromArray(kernelSize, kernelSize, window.cv.CV_32FC1, arr);
      window.cv.filter2D(src, dst, window.cv.CV_8U, kernel, new window.cv.Point(-1, -1), 0, window.cv.BORDER_DEFAULT)
      // window.cv.imshow("outputCanvas2", dst);
      this.images.push({
        id: this.images.length,
        dst,
      })
    ...
    bilateralFiltering() {
      let src = window.cv.imread(this.$refs.defaultImageRef);
      let dst = new window.cv.Mat();
      window.cv.cvtColor(src, src, window.cv.COLOR_RGBA2RGB, 0);
      window.cv.bilateralFilter(src, dst, 9, 75, 75, window.cv.BORDER_DEFAULT);
    ...
    rerenderImages() {
      ...
      this.images.forEach((img, index) => {
        ...
        setTimeout(() => {
          console.log('teraz')
          window.cv.imshow(`abc${index}`, img.dst);
        }, 0)
      })
    }
  },
